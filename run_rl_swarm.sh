...
DEFAULT_HOST_MULTI_ADDRS="/ip4/0.0.0.0/tcp/38353"
...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if open http://localhost:3020 2> /dev/null; then
        echo_green ">> Successfully opened http://localhost:3020 inyour default browser."
    else
        echo ">> Failed to open http://localhost:3020 Please open it manually."
    fi
    ...
    while true; do
        STATUS=$(curl -s "http://localhost:3020/api/get-api-key-status?orgId=$ORG_ID")
