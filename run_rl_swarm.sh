...
DEFAULT_HOST_MULTI_ADDRS="/ip4/0.0.0.0/tcp/38352"
...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if open http://localhost:3019 2> /dev/null; then
        echo_green ">> Successfully opened http://localhost:3019 inyour default browser."
    else
        echo ">> Failed to open http://localhost:3019 Please open it manually."
    fi
    ...
    while true; do
        STATUS=$(curl -s "http://localhost:3019/api/get-api-key-status?orgId=$ORG_ID")
