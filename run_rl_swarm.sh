...
DEFAULT_HOST_MULTI_ADDRS="/ip4/0.0.0.0/tcp/38351"
...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if open http://localhost:3018 2> /dev/null; then
        echo_green ">> Successfully opened http://localhost:3018 inyour default browser."
    else
        echo ">> Failed to open http://localhost:3018 Please open it manually."
    fi
    ...
    while true; do
        STATUS=$(curl -s "http://localhost:3018/api/get-api-key-status?orgId=$ORG_ID")
